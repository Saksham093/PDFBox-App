...
import com.sam_p.pdfbox.helper.ScrollHandle
...
class PDFViewerActivity : AppCompatActivity() {
    ...
    public override fun onCreate(bundle: Bundle?) {
        ...
        val intent = intent
        pdfFileLocation =
            intent.getStringExtra(com.sam_p.pdfbox.data.Constants.PDF_LOCATION)
        showRemoveAds = intent.getBooleanExtra(
            com.sam_p.pdfbox.data.Constants.SHOW_REMOVE_ADS,
            false
        )
        uri = intent.data
    ...
    public override fun onActivityResult(i: Int, i2: Int, intent: Intent?) {
        ...
        if (i == 7 && i2 == -1) {
            pdfView!!.jumpTo(intent!!.getIntExtra(com.sam_p.pdfbox.data.Constants.PAGE_NUMBER, pdfView!!.currentPage) - 1, true)
        }
    }
    ...
    private fun toggle() {
        ...
        show()
        if (AUTO_HIDE) {
            delayedHide(com.sam_p.pdfbox.data.Constants.AUTO_HIDE_DELAY_MILLIS)
        }
    }
